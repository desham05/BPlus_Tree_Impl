...
public class BTreeFile extends IndexFile implements GlobalConst {
	...
	{
		PageId pageno = headerPage.get_rootId();    									// returns root Id of the BTREE (-1 in case of empty BTREE)
		if (pageno.pid == INVALID_PAGE) { 												// check if the BTREE is empty
			BTLeafPage leafPage = new BTLeafPage(headerPage.get_keyType()); 			// If empty create a new leaf page
			PageId newLeafPageId = leafPage.getCurPage();
			pinPage(newLeafPageId);
			leafPage.setNextPage(new PageId(INVALID_PAGE));								// set next or right link page to invalid  i.e. -1
			leafPage.setPrevPage(new PageId(INVALID_PAGE));								// set previous or the left link page to invalid i.e. -1
			leafPage.insertRecord(key,rid);												// insert passed record in the leaf page
			unpinPage(newLeafPageId);
			updateHeader(newLeafPageId);												// update header information 
		}else{
			PageId currPageId = headerPage.get_rootId();
			KeyDataEntry newEntry = _insert(key, rid, currPageId);						// insert the entry into the tree at proper position
			if(newEntry != null){														// returns entry if the root node is full
				BTIndexPage newIndexPage = new BTIndexPage(headerPage.get_keyType());   // create new root page    
				PageId newRootPageId = newIndexPage.getCurPage();
				pinPage(newRootPageId);
				newIndexPage.insertKey(newEntry.key, ((IndexData)newEntry.data).getData());
				newIndexPage.setPrevPage(headerPage.get_rootId());
				unpinPage(newRootPageId);
				updateHeader(newRootPageId);											// update new header information 
			}
		}
	}
	...
	{
		KeyDataEntry newEntry = new KeyDataEntry(key, currentPageId);
		Page page;
		page = pinPage(currentPageId); 
		BTSortedPage currentPage = new BTSortedPage(page,headerPage.get_keyType());                                    // create sorted page with current referred page
		if(currentPage.getType() == NodeType.INDEX){																   // check whether page is of type index
			BTIndexPage currIndexPage = new BTIndexPage(currentPage,headerPage.get_keyType());
			PageId currIndexPageId = currIndexPage.getCurPage();
			PageId newPageId = currIndexPage.getPageNoByKey(key);													   // search for the page id in which the key to be inserted
			pinPage(currIndexPageId);
			newEntry = _insert(key, rid, newPageId); 																   // insert the entry into the tree at proper position
			if(newEntry == null){																					   
				return null;
			}else{
				if(headerPage.get_rootId().pid == currIndexPageId.pid){												   // to enter data in header page
					BTIndexPage rootIndexPage = new BTIndexPage(currIndexPageId, headerPage.get_keyType());			   // get the header page
					rootIndexPage.insertKey(newEntry.key, ((IndexData)newEntry.data).getData());					   // insert the value in header page
					return null;
				}else if(currIndexPage.available_space() >= BT.getKeyDataLength(newEntry.key, NodeType.INDEX)){		   // check whether space available in index page			
					currIndexPage.insertKey(key, currentPageId);
					unpinPage(currIndexPageId);
					return null;
				}else{																									// if the index page is full
					BTIndexPage newIndexPage = new BTIndexPage(headerPage.get_keyType());
					pinPage(newPageId);
					KeyDataEntry tmpKeyDataEntry;
					RID delRid = new RID();
					for(tmpKeyDataEntry = currIndexPage.getFirst(delRid);tmpKeyDataEntry!=null;tmpKeyDataEntry = currIndexPage.getFirst(delRid)){     // copy all the records in newly created page
						try {
							newIndexPage.insertRecord(tmpKeyDataEntry);																				  // insert all the records to new page
							currIndexPage.deleteRecord(delRid);																						  // delete all the records from old page which is full
						} catch (Exception e) {
							e.printStackTrace();
						}
					}

					for(tmpKeyDataEntry = newIndexPage.getFirst(delRid);tmpKeyDataEntry!=null;tmpKeyDataEntry = newIndexPage.getFirst(delRid)){       // copy first half records on old page and delete it from new page
						try {
							currIndexPage.insertRecord(tmpKeyDataEntry);
							newIndexPage.deleteSortedRecord(delRid);
						} catch (Exception e) {
							e.printStackTrace();
						}

						if(currIndexPage.numberOfRecords() == newIndexPage.numberOfRecords()){                                                       // equally divide records if even records present
							break;
						}else if(currIndexPage.numberOfRecords() == (newIndexPage.numberOfRecords()+1)){											 // divide records when odd records 
							break;
						}
					}

					if(BT.keyCompare(newEntry.key, tmpKeyDataEntry.key) > 0){																		 // comparison to put new entry to which page
						newIndexPage.insertKey(key, currentPageId);
					}else{
						currIndexPage.insertKey(key, currentPageId);
					}

					unpinPage(currIndexPageId);
					newEntry = newIndexPage.getFirst(delRid);
					newIndexPage.setPrevPage(currIndexPageId);																						 // link previous page to current page		
					try {
						newIndexPage.deleteRecord(delRid);
					} catch (Exception e) {
						e.printStackTrace();
					}
					unpinPage(newPageId);
					((IndexData)newEntry.data).setData(newPageId);
					return newEntry;
				}
			}
		}else if(currentPage.getType() == NodeType.LEAF){																						     // check whether page is of type index
			BTLeafPage currLeafPage = new BTLeafPage(currentPage,headerPage.get_keyType());															 // fetch current page reference
			PageId currLeafPageId = currLeafPage.getCurPage();
			pinPage(currLeafPageId);
			if(currLeafPage.available_space() >= BT.getKeyDataLength(newEntry.key, NodeType.LEAF)){												     // check if the space is present in the current leaf page
				currLeafPage.insertRecord(key, rid);
				unpinPage(currLeafPageId);
				return null;
			}else{																																	
				BTLeafPage newLeafPage = new BTLeafPage(headerPage.get_keyType());																	 // create new leaf page as no space for new record
				PageId newPageId = newLeafPage.getCurPage();
				pinPage(newPageId);
				newLeafPage.setNextPage(new PageId(INVALID_PAGE));																					 // set next link of new page to invalid page 
				newLeafPage.setPrevPage(currLeafPageId);																							 // set previous page link to current page
				currLeafPage.setNextPage(newPageId);																								 // set next page link to new page created
				KeyDataEntry tmpKeyDataEntry;
				RID delRid = new RID();
				for(tmpKeyDataEntry = currLeafPage.getFirst(delRid);tmpKeyDataEntry!=null;tmpKeyDataEntry = currLeafPage.getFirst(delRid)){          // move all the records to new page
					try {
						newLeafPage.insertRecord(tmpKeyDataEntry);
						currLeafPage.deleteSortedRecord(delRid);
					} catch (Exception e) {
						e.printStackTrace();
					}
				...
				for(tmpKeyDataEntry = newLeafPage.getFirst(delRid);tmpKeyDataEntry!=null;tmpKeyDataEntry = newLeafPage.getFirst(delRid)){			  // move half records to previous page	
					try {
						currLeafPage.insertRecord(tmpKeyDataEntry);
						newLeafPage.deleteSortedRecord(delRid);
					} catch (Exception e) {
						e.printStackTrace();
					}

					if(currLeafPage.numberOfRecords() == newLeafPage.numberOfRecords()){															  // equally divide records if even records present	
						break;
					}else if(currLeafPage.numberOfRecords() == (newLeafPage.numberOfRecords()-1)){													  // divide records when odd records	
						break;
					}
				}
				...
				newEntry = new KeyDataEntry(tmpKeyDataEntry.key, newPageId);
				unpinPage(newPageId);
				return newEntry;
			}
		}else{
	...
	private boolean NaiveDelete(KeyClass key, RID rid)
		...
		KeyDataEntry deleteEntry;

		leafPage = findRunStart(key,currRid);															// search for the page where entry to be deleted is present
		if(leafPage == null){																			// record not found
			return false;
		}else{
			deleteEntry = leafPage.getCurrent(currRid);													// take the record found as entry
			PageId nxtpage;
			while(true){
				while(deleteEntry == null){																// entry not preset in the current page
					nxtpage = leafPage.getNextPage();													// fetch next page to search entry
					unpinPage(leafPageId);
					if(nxtpage.pid == INVALID_PAGE){
						return false;
					}
					leafPageId = nxtpage;
					deleteEntry = leafPage.getFirst(rid);
				}
				if(BT.keyCompare(deleteEntry.key, key) > 0){
					break;
				}

				if(leafPage.delEntry(new KeyDataEntry(key, rid))== true){								// delete the entry found in page
					unpinPage(leafPageId);
					deleteEntry = leafPage.getCurrent(currRid);											// get the remaining duplicate entries if present to delete
					if(deleteEntry != null){															// return not null if entry present	
						pinPage(leafPageId);
						continue;																		// continue to delete entries
					}else{
						break;																			// break if the entry not found
					}
				}else{
					nxtpage = leafPage.getNextPage();													// go to the next page 
